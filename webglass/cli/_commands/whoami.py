"""``webglass-cli whoami`` — the smallest identity probe.
...
Identity is entirely config-driven: editing ``culture.yaml`` (``suffix`` /
``backend`` / ``model``) changes what ``whoami`` reports, with no code change
required.
"""
